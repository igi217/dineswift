...
import type { ExpenseType } from "@/db/index.type";
import { useExpenseType } from "@/hooks/db_query";
import { ScrollArea } from "@radix-ui/react-scroll-area";
import { ColumnDef } from "@tanstack/react-table";

import { DataTable } from "@/components/ui/data-table";
import ExpenseTypeAction from "@/components/ui/expensetype-action";
import { ExpenseTypeTableToolbar } from "@/components/ui/expensetype-table-toolbar";
...
export const columns: ColumnDef<ExpenseType>[] = [
  ...
  },
  {
    ...
    header: "Actions",
    enableHiding: false,
    cell: ({ row }) => <ExpenseTypeAction id={row.getValue("_id")} />,
  },
];
        ...
        <ScrollArea className="mt-5 p-3">
          <Card>
            <CardHeader>
              ...
              <CardDescription>List of Expense Types</CardDescription>
            </CardHeader>
            <CardContent>
              <DataTable
                toolbar={(table) => <ExpenseTypeTableToolbar table={table} />}
                data={expensetypes ?? []}
                columns={columns}
              />
            </CardContent>
