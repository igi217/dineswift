...
export default function CustomerTypeAction(props: CustomerTypeAction) {
  const navigate = useNavigate();

  async function handleEdit() {
    let link = resolveRoute(CustomerTypeEdit.route, props);

    navigate(link);
  }

  async function handleDelete() {
    let customertype: any = await db.get(props.id);

    await db.remove(customertype);
