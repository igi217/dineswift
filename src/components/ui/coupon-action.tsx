...
export default function CouponAction(props: CouponAction) {
  const navigate = useNavigate();

  async function handleEdit() {
    let link = resolveRoute(CouponEdit.route, props);

    navigate(link);
  }

  async function handleDelete() {
    let coupon: any = await db.get(props.id);

    await db.remove(coupon);
